...
## 8. Error Handling and Propagation (`NaN` Semantics)

Because `CoreGraph` avoids object allocation to achieve zero-GC execution, exceptions are strongly discouraged inside the hot-path `calculate()` methods. Throwing exceptions forces the JVM to allocate stack-traces, destroying performance.

Instead, the engine utilizes primitive `Double.NaN` (Not a Number) object semantics to propagate errors or invalid states.

*   **Source Nodes:** If an upstream data feed is disconnected or invalid, you should `.update(Double.NaN)`.
*   **Calc Nodes:** If logic fails (e.g., division by zero), return `Double.NaN`.
*   **Propagation:** The framework (`AbstractFnN`, `AbstractFn2`) automatically intercepts inputs. If any single input dependency is `NaN`, the function automatically short-circuits and returns `NaN` downstream, collapsing the invalidity across the entire dependent sub-graph gracefully without executing cycles.
